...
DELIMITER $$
USE `isell_db`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `GET_BREAKEVEN_PRICE`(_product_code VARCHAR(45)  CHARSET utf8, pcomp_id INT, usd_ratio DOUBLE, _self_price DOUBLE) RETURNS DECIMAL(10,2)
    DETERMINISTIC
BEGIN
    ...
    DECLARE _promo_price DECIMAL(10,2);
