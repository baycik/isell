...
class DocumentList extends Catalog
{
	...
	public function listFetch(int $offset, int $limit, string $sortby = null, string $sortdir = null, array $filter = null, string $mode = null, string $colmode = null)
	{
		...
		$sql = "
			...
			doc_id,
			CASE
				WHEN doc_type = 1 THEN 'DocSell'
				WHEN doc_type = 2 THEN 'DocBuy'
				WHEN doc_type = 3 THEN 'DocServiceSell'
				WHEN doc_type = 4 THEN 'DocServiceBuy'
				WHEN doc_type = 5 THEN 'DocAgentSell'
				ELSE 'UnknownDocType'
			END doc_extension,
			dl.cstamp,
