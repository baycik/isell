...
class Document extends Data
{
	...
	protected function getNextDocNum($doc_type, $creation_mode)
	{ //Util
	...
	protected function isDebtLimitExceeded()
	{
		...
		$debt_limit = 		(int) $this->Base->pcomp('debt_limit')??$prefs['default_debt_limit']??0;
		$deferment = 		(int) $this->Base->pcomp('deferment')??$prefs['default_debt_defferment']??0;
		$debt_limit_restr = (int) $this->Base->pcomp('debt_limit_restr')??$prefs['restrictive_debt_limit']??0;
		$deferment_restr = 	(int) $this->Base->pcomp('deferment_restr')??$prefs['restrictive_debt_defferment']??0;
		if( !$debt_limit && !$deferment && !$debt_limit_restr && !$deferment_restr ){
			return false;
		}
		...
		if( $debt_limit_restr > 0 ){
			$debt_account = $this->Base->Accounts->getAccountBalance(361, $pcomp_id, $deferment_restr);
			$off_limit = $footer['total'] + $debt_account['balance'] - $debt_limit_restr;
			if ( $off_limit > 100) {
				$this->Base->msg("🛑 Стоп отгрузки. Лимит долга превышен на " . number_format($off_limit, 2, '.', ' ') . "{$footer['curr_symbol']}!\n");
				return true;
			}
			if ( $debt_account['expired_balance'] > 0 ) {
				$this->Base->msg("🛑 Стоп отгрузки. Имеется просроченная задолженность " . number_format($debt_account['expired_balance'], 2, '.', ' ') . "{$footer['curr_symbol']}!\n");
				return true;
			}
		}
		if( $deferment > 0 ){
			...
			if ( $off_limit > 0 ) {
				$this->Base->msg("⚠️ Лимит долга превышен на " . number_format($off_limit, 2, '.', ' ') . "{$footer['curr_symbol']}!\n");
				return false;
			}
			if ( $debt_account['expired_balance'] > 0 ) {
				$this->Base->msg("⚠️ Имеется просроченная задолженность " . number_format($debt_account['expired_balance'], 2, '.', ' ') . "{$footer['curr_symbol']}!\n");
				return false;
			}
		}
		return false;
	...
	protected function alterEntry($action, $doc_entry_id, $new_quantity = NULL, $new_invoice = NULL, $new_party_label = NULL)
	...
	public function insertView($view_type_id, $doc_id = null)
	{
		...
		$this->Base->query("INSERT INTO document_view_list SET doc_id='$doc_id', html='', view_type_id='$view_type_id', view_efield_values=IF(" . (empty($efields) ? 0 : 1) . ",'$efields',NULL), tstamp='$cstamp', view_num='$view_num', view_role='{$view_type_props['view_role']}'");
	...
	protected function makeTransaction($acc_debit_code, $acc_credit_code, $amount, $description, $trans_role)
	{
		...
		if (!$trans_id) { //Transaction does not exists
			...
			$this->Base->Accounts->updateTransaction($trans_id, array('amount' => (float) $amount, 'amount_alt' => $amount_alt, 'description' => $description, 'trans_role' => $trans_role, 'doc_id' => $doc_id));
		}
