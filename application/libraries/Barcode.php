...
//        }
    ...
    protected function _drawBarsHtml() {
        $HEIGHT=30;
        $WIDTH = $this->scale;
        $this->html='';
        foreach ($this->_bars as $bar) {
            ...
            for ($i = 1; $i <= strlen($bar); $i++) {
                if (substr($bar, $i - 1, 1) === '1'){
                    ...
                    $this->html.="<div style='background-color:black;height:{$HEIGHT}px;width:{$WIDTH}px;display:inline-block'></div>";
                } else{
                    $this->html.="<div style='background-color:white;height:{$HEIGHT}px;width:{$WIDTH}px;display:inline-block'></div>";
                }
            }
        }
    }
...
$barcode=new Barcode($_GET['barcode'],1,'html');
