...
class Reports_stock_movements extends Catalog
{
    ...
    public function viewGet()
    {
        ...
        $sql_tmp_create1 = "CREATE TEMPORARY TABLE tmp_summary_sell_buy (PRIMARY KEY (product_code)) AS(
            ...
            WHERE
                doc_type IN (1,2) AND is_commited=1 AND notcount=0 AND cstamp<'$this->fdate' $reclamation_filter $active_filter
        ...
        $sql_summary = "
            ...
            GROUP BY
		        $this->group_by $this->group_by2_comma
            HAVING
                total_stock_isum OR total_buy_sum OR total_sell_sum OR total_stock_fsum
            ORDER BY
        ...
        if ($this->show_entries) {
            $rows = $this->get_list("SELECT * FROM tmp_stock_movements WHERE stock_isum OR buy_sum OR sell_sum OR stock_fsum ORDER BY group_by,_product_code");
            foreach ($rows as $row) {
                $this->clear_zero($row);
            }
        }
