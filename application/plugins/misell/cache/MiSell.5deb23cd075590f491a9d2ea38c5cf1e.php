...
<html>
    <head>
	    ...
	    #autocomplete .ui-li-static.ui-li-has-thumb{
		padding-left: 5.3em;
	    }
	    #autocomplete .ui-li-static{
		white-space: normal;
	    }
        </style>
        <?php echo $scripts;?>
		    ...
		    return $(window).width()+'x'+$(window).height();
                ...
                client: {
                    ...
                    show: function () {
                        ...
                        html += "<li style='white-space:normal'><b>Долг: </b>" + (selected_client.company_debt || '') + "</li>";
				...
				return false;
                    ...
                    show: function () {
                        ...
                        $.each(_orderCache, function (pcode, entry) {
                            if (entry) {
                                html += '<li>';
                                html += '<span style="width:40%;display:inline-block;overflow:hidden;"><a href="#" onclick="App.order.entry.remove(\'' + pcode + '\')" class="ui-btn ui-corner-all ui-icon-delete ui-btn-icon-notext ui-btn-b ui-btn-inline">Delete</a>  ' + pcode + '</span>';
                                html += '<span style="width:30%;display:inline-block;overflow:hidden;"><a href="#" onclick="App.order.entry.edit(\'' + pcode + '\')" class="ui-btn ui-corner-all ui-mini ui-btn-inline ui-btn-b">' + entry.qty + '</a>' + entry.unit + '</span>';
                                html += '<span style="width:30%;display:inline-block;overflow:hidden; padding-bottom: 0.7em;">' + (App.format(entry.price) || '?') + '</span>';
                    ...
                    loadSuggestions: function (q) {
                        ...
                        $.get("./suggest", {q: q, parent_id: _selectedStockCat, company_id: App.store('selected_client').company_id}, function (response) {
                            ...
                            $.each(suggested, function (i, val) {
                                ...
                                html += "<img src='../../Storage/image_flush/100x100/dynImg/"+val.product_img+"' onclick='return App.order.imgPopupShow(this);' style='float:left'>";
                                html += "<span style='color:#666;'>" + val.code + "</span>";
                                html += " <b>" + val.price + "</b> [" + val.product_quantity + "] <br>" + val.name + "</li>";
                            });
                            $ul.html(html);
                            $ul.listview("refresh");
    ...
    <body>
        ...
        <div data-role="page" id="orderPage" style="overflow-y: auto;">
            ...
            <div data-role="main" class="ui-content">
				...
				<div data-role="popup" id="popupProductZoom" class="photopopup" data-overlay-theme="a" data-corners="false" data-dismissible="true" data-tolerance="30,15">
		    <a href="#" data-rel="back" class="ui-btn ui-corner-all ui-shadow ui-btn-a ui-icon-delete ui-btn-icon-notext ui-btn-right">Close</a>
		    <img src="application/plugins/MiSell/">
		</div>
                <!--QUANTITY DIALOG-->
                ...
                <!--AUTOSUGGEST-->
		<input type="search"  data-clear-btn="true" id="suggest" placeholder="Поиск товаров..." data-theme="b"/>
                <ul id="autocomplete"
                    data-role="listview" 
                    data-inset="false" 
                    data1-filter="true" 
